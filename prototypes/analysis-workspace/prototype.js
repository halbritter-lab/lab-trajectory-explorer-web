import { derivedPanel, bindDerived } from './derived-parameters.js';
...
function analysisResults() {
  if (!state.lastResult) return '<p class="muted analysis-placeholder">Das Ergebnis erscheint nach dem Berechnen unter der Konfiguration des Gruppenvergleichs.</p>';
  ...
  return `<section class="card analysis-results" aria-labelledby="results-title"><div class="card-header"><div><p class="eyebrow">${fresh()?'Ergebnis':'Bisheriges Ergebnis'}</p><h2 id="results-title" tabindex="-1">Ergebnis des Gruppenvergleichs</h2><p class="muted">${escape(config.response)} · ${n} Personen · ${n*6} Messungen · Alter: ${effect(config.age)} · Geschlecht: ${effect(config.sex)}</p></div><span class="badge ${fresh()?'green':'amber'}">${fresh()?'Beispielergebnis':'Veraltet'}</span></div>
...
function analysisPage() {
  ...
  return `<div class="page-heading"><div><p class="eyebrow">03 / Kohortenmodelle</p><h1>Genotypen im Verlauf vergleichen</h1><p class="muted">Mehrere Personen gemeinsam modellieren. Auswertungen einzelner Verläufe findest du unter „Verläufe“.</p></div></div>
    ${analysisConfig()}${analysisResults()}`;
}
function render() {
  root.innerHTML = `<header class="topbar"><div class="brand"><svg width="27" height="27" viewBox="0 0 28 28" aria-hidden="true"><path d="M3 4v21h23M5 10l6 3 6-5 8 9" fill="none" stroke="currentColor" stroke-width="2"/></svg>Trajektorien</div><nav class="nav" aria-label="Hauptnavigation">${['Daten','Verläufe','Kohortenmodelle'].map(page=>`<button data-page="${page}" ${state.page===page?'aria-current="page"':''} ${!state.loaded&&page!=='Daten'?'disabled':''}>${page}</button>`).join('')}</nav><div class="top-actions"><span class="badge">Entwurf · Beispieldaten</span></div></header><main id="content" class="workspace">${state.page==='Daten'?dataPage()+(state.loaded?derivedPanel():''):state.page==='Verläufe'?trajectoriesPage():analysisPage()}</main><footer class="prototype-footer">Klick-Prototyp · Alle Daten und Ergebnisse sind illustrativ · Nur für Forschungszwecke</footer>`;
...
document.addEventListener('click', event => {
  ...
  if (action==='start'||action==='projection-start') { state.started=true; state.page='Kohortenmodelle'; }
