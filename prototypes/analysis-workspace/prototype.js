...
function render() {
  root.innerHTML = `<header class="topbar"><div class="brand"><svg width="27" height="27" viewBox="0 0 28 28" aria-hidden="true"><path d="M3 4v21h23M5 10l6 3 6-5 8 9" fill="none" stroke="currentColor" stroke-width="2"/></svg>Trajektorien</div><nav class="nav" aria-label="Hauptnavigation">${['Daten','Verläufe','Kohortenmodelle'].map(page=>`<button data-page="${page}" ${state.page===page?'aria-current="page"':''} ${!state.loaded&&page!=='Daten'?'disabled':''}>${page}</button>`).join('')}</nav><div class="top-actions"><span class="badge">Entwurf · Beispieldaten</span></div></header><main id="content" class="workspace">${state.page==='Daten'?dataPage():state.page==='Verläufe'?trajectoriesPage():analysisPage()}</main><footer class="prototype-footer">Klick-Prototyp · Alle Daten und Ergebnisse sind illustrativ · Nur für Forschungszwecke</footer>`;
