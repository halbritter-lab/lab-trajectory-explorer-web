...
describe('workspace workbook', () => {
  it('preserves UTC import dates in a negative-offset timezone', () => {
    ...
    try {
      ...
      expect(records(workbook,'measurements')[0].Datum).toBe('2024-01-01')
      expect(records(workbook,'raw_measurements')[0].Datum).toBe('2024-01-01')
  ...
  it('roundtrips the visible scope with actual prepared summaries and derived provenance', () => {
    ...
    expect(records(workbook,'measurements')).toEqual(expect.arrayContaining([expect.objectContaining({PatientID:'001-A',Bezeichnung:'Marker|one',Einheit:'mg/L',WertNum:10,origin:'imported'}),expect.objectContaining({Bezeichnung:'Derived eGFR',origin:'derived',formula:'ekfc-2021'})]))
  ...
  it('intersects individual selection with visible IDs and rejects empty or missing summaries', () => {
    ...
    expect(() => workspaceWorkbookBytes({...input,patientId:'hidden'})).toThrow(/Patient|Auswahl/)
    expect(() => workspaceWorkbookBytes({...input,parameterKeys:[]})).toThrow(/Parameter/)
    expect(() => workspaceWorkbookBytes({...input,cohortRows:[]})).toThrow(/Zusammenfassung/)
  })
})
...
describe('chart serialization', () => {
  it('embeds visible parameter/unit, legend and context and standalone text styles', () => {
    ...
    svg.setAttribute('data-export-context','Kalenderdatum · Gruppe: Zentrum · 2 Personen')
    ...
    svg.append(axis)
    const result = exportChartSvg(svg,'Sehr langer Marker [mg/L]')
    ...
    expect(text).toContain('Sehr langer Marker [mg/L]')
    expect(text).toContain('Kalenderdatum')
  ...
  it('adds visible research attribution without mutating the displayed chart', () => {
    ...
    expect(exported.svg).toContain('Nur für Forschungszwecke')
    expect(exported.svg).toContain('Marker &lt;sample&gt;')
    expect(exported.height).toBeGreaterThan(200)
    expect(svg.outerHTML).not.toContain('Forschungszwecke')
    expect(() => exportChartSvg(document.createElementNS(svg.namespaceURI,'svg') as SVGSVGElement,'empty')).toThrow(/Diagramm/)
