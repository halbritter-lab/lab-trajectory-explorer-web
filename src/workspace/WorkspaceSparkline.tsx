...
export function WorkspaceSparkline({ cell, measurements, patientId, label, domain, fit }: {
  ...
  return <svg className="wt-sparkline" viewBox="0 0 218 85" role="img" aria-label={`Messverlauf Person ${patientId}, ${label}: ${cell.points.length} Messwerte; gemeinsame Skala, Tage seit erster Messung`}>
    <title>{`Person ${patientId}, ${label}: ${cell.points.map((point, i) => `${formatWorkspaceDate(point.date)}: ${boundedPrefix(sources[i]?.wertOperator)}${formatWorkspaceNumber(point.value)}`).join('; ')}`}</title>
    ...
    <text x="34" y="78">0</text><text x="198" y="78" textAnchor="end">{formatWorkspaceNumber(domain.days)} Tage</text>
