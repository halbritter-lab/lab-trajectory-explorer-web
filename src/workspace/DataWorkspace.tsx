...
export function DataWorkspace({ onBrowse }: { onBrowse: (patientId?: PatientId) => void }) {
  ...
  return <div className="data-workspace stack">
    ...
    <section className="card"><h2>1 · Import a dataset</h2><p>CSV or Excel; sheets containing lab values, events and attributes are read together. All data stays in this session.</p>
      ...
      {data.rawRows.length > 0 && <><p><strong>{data.fileName ?? 'Dataset'}</strong></p><div className="data-metrics"><span><strong>{data.patients.length}</strong> patients</span><span><strong>{data.rawRows.length}</strong> imported values</span><span><strong>{data.parameters.filter(p => !p.derived).length}</strong> Parameters / units</span><span><strong>{data.events.length}</strong> events</span></div>
    ...
    {data.rawRows.length > 0 && <><section className="card"><h2>2 · Demographics and data quality</h2><p>{missingSex} patients without resolved sex · {missingAge} without age at the first lab date · {resolution.conflicts.length} conflicts</p>
      ...
      <details><summary>Review and edit patients ({data.patients.length})</summary><div className="table-scroll"><table><thead><tr><th>Person</th><th>Sex</th><th>Age at first measurement</th><th>Quality</th><th>Action</th></tr></thead><tbody>{data.patients.map(p => <tr key={JSON.stringify(p.id)}><th><button onClick={() => onBrowse(p.id)}>{p.label}</button></th><td>{p.attributes.sex ?? 'Missing'}</td><td>{p.baselineAge ?? 'Missing'}</td><td>{resolution.conflicts.filter(c => c.patientId === p.id).length ? 'Conflict' : !p.attributes.sex || p.baselineAge === null ? 'Incomplete' : 'Complete'}{data.manualDemographics[patientIdKey(p.id)] && Object.keys(data.manualDemographics[patientIdKey(p.id)]).length > 0 ? ' · manual' : ''}</td><td><button aria-label={`Edit demographics: ${p.label}`} onClick={() => edit(p.id)}>Edit</button></td></tr>)}</tbody></table></div></details>
      ...
      {!!computed.length && <details><summary>Inspect computed values ({computed.length})</summary><div className="table-scroll"><table><thead><tr><th>Person</th><th>Date</th><th>Parameter</th><th>Value</th></tr></thead><tbody>{computed.map((r, i) => <tr key={i}><td>{r.patientId}</td><td>{r.labDatum?.toISOString().slice(0, 10)}</td><td>{r.bezeichnung}</td><td>{r.wertOperator === '=' ? '' : r.wertOperator}{r.wertNum?.toFixed(1)} {r.einheit}</td></tr>)}</tbody></table></div></details>}
