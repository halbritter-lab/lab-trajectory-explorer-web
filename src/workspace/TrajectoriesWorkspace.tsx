...
function CellSummary({ cell, fit, measurements }: { cell: CohortCell; fit: boolean; measurements: LabRow[] }) {
  ...
  const qualityText = quality?.label === 'no values' ? 'No numeric measurements' : quality?.label === 'no fit values' ? 'No fitted measurements' : quality?.label === '< 1 yr' ? 'Follow-up < 1 year' : quality?.label
  return <div className="wt-cell-summary">
    ...
    {fit && <><span>{Number.isFinite(cell.slope) ? `OLS: ${formatWorkspaceNumber(cell.slope)} ${cell.einheit ?? ''}/year · R² ${formatWorkspaceNumber(cell.r2)}` : 'No fit available'}</span><span>{cell.nFitted} fitted measurements · {cell.fittedSpanDays} days</span>{quality && <span className={quality.caveat ? 'wt-warning' : 'wt-muted'}>{qualityText}{quality.caveat ? ' · uncertain slope' : ''}</span>}</>}
  ...
  return <div className="wt-workspace">
    ...
    <section className="card wt-controls" aria-label="Shared selection">
      ...
      <div className="wt-control-grid">
        ...
        <label>Filter group<select value={group} disabled={!groupBy} onChange={event => setGroup(event.target.value)}><option value="">All groups</option>{groupBy && groups.map(value => <option key={value}>{value}</option>)}</select></label>
    ...
    <div className="wt-toolbar" aria-label="View"><button aria-pressed={mode === 'table'} onClick={() => setMode('table')}>Table</button><button aria-pressed={mode === 'overlay'} onClick={() => setMode('overlay')}>Overlay</button><button aria-pressed={mode === 'detail'} disabled={!current} onClick={() => setMode('detail')}>Individual patient</button></div>
    <details className="card"><summary>Display and analysis</summary><p>General exploration: global OLS per patient and parameter, unweighted individual measurements, no AKI or event exclusions, and no time aggregation. Slopes are per year. Changing the axis affects the display, not the calculation.</p><div className="wt-toolbar">{parameters.map(p => <label key={p.key}><input type="checkbox" checked={fitKeys.includes(p.key)} onChange={() => toggleFit(p.key)} /> OLS, slope and R²: {p.label}</label>)}</div></details>
    ...
    {mode === 'table' && parameters.length > 0 && <div className="wt-toolbar"><button onClick={() => moveColumns(-1)}>Scroll columns left</button><label>Jump to parameter<select defaultValue="" onChange={event => jumpParameter(event.target.value)}><option value="" disabled>Choose parameters …</option>{parameters.map(parameter => <option key={parameter.key} value={parameter.key}>{parameter.label}</option>)}</select></label><button onClick={() => moveColumns(1)}>Scroll columns right</button><span className="wt-muted">Shared scale per parameter; time since first measurement. Patient IDs remain visible while scrolling horizontally.</span></div>}
