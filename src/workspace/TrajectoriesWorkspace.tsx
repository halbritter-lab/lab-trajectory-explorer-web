...
export function TrajectoriesWorkspace({ data, requestedPatientId }: { data: WorkspaceData; requestedPatientId?: PatientId | null }) {
  ...
  return <div className="wt-workspace">
    ...
    {mode === 'table' && parameters.length > 0 && <div className="wt-toolbar"><button onClick={() => moveColumns(-1)}>Scroll columns left</button><label>Jump to parameter<select defaultValue="" onChange={event => jumpParameter(event.target.value)}><option value="" disabled>Choose parameters …</option>{parameters.map(parameter => <option key={parameter.key} value={parameter.key}>{parameter.label}</option>)}</select></label><button onClick={() => moveColumns(1)}>Scroll columns right</button><span className="wt-muted">{scaleMode === 'shared' ? 'Shared parameter scales.' : 'Zoomed visible-value scales.'} Time since first measurement; patient IDs stay visible while scrolling.</span></div>}
